...
class fNIRSGraphDataset(Dataset):
    ...
    def compute_stats(self, indices: Optional[Sequence[int]] = None) -> Dict[str, torch.Tensor]:
        """Per-channel feature stats. Pass `indices` to restrict to a fold's training subset
        — required for leak-free k-fold/LOSO normalization (see SG_vs_ST_validation_comparison.md §7).
        Default `indices=None` retains full-dataset behavior for callers that handle splits separately.
        """
