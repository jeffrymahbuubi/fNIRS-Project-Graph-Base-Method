...
class StandardizeGraphFeatures(BaseTransform):
    def __init__(
        ...
        # x is [C, T] z-scored per-channel per-trial in _build_graph(); no x normalization needed
...
def get_transforms(
    ...
    use_rwpe: bool = False,  # not applicable to ST model; kept for API compat, silently ignored
    ...
    if augment:
        if feature_mask_p > 0.0:
            # Operates on x=[C,T]: randomly zeros individual timepoint values per channel
