...
>
> **Date created:** 2026-05-10 · **Last updated:** 2026-05-14
>
> **Progress log (2026-05-14 — P1.7 channel-ablation validation DONE; new paper headline):**
> - ✅ **P1.7 channel-ablation validation DONE.** 24 runs complete (3 chromos × 4 K × 2 mt) at `research/experiments/20260513/{mt2,mt4}/`. Code: `--channel_subset` flag in `src/core_st/main.py` + `dataset.py` + `config.py`; 15 unit tests in `tests/core_st/test_channel_subset.py` (all pass; bit-identical-to-no-feature path when `channel_subset=None`).
> - ✅ **NEW PAPER HEADLINE LOCKED: ST × HbO × LOSO × mt=2 × K=12 = F1 0.8529** (sens 1.000, spec 0.697, CM [[46, 20], [0, 58]]). +3.60 pp over HbO K=23 baseline; +1.23 pp over the prior global best HbR mt2 K=23 = 0.8406 (which is no longer the headline). HbO chromophore matches the statistical-analysis arm; perfect sensitivity is clinically defensible; provenance traces directly to the mt2 differential XAI.
> - ✅ **K-consistency analysis:** K=12 helps in 5/6 (chromo × mt) cells with mean Δ-F1 +2.52 pp — the most beneficial subset across configurations (vs +1.85 pp at K=16 and +1.29 pp at K=8). Highest mean F1 of any tested K (0.8401).
> - ✅ **Secondary number:** HbO mt4 K=8 = 0.8672 (highest raw F1 across the 24-cell sweep; sens 0.957, spec 0.780). Reported as a sensitivity-analysis row in §III.C.2 table, not the headline (trades anxiety detection for specificity; sharp local max).
> - ✅ **mt2 K=23 baselines reproduce 20260509 bit-identically** for all 3 chromophores; mt4 K=23 reproduces for HbO + HbR. HbT mt4 K=23 anomaly (20260513 outlier at 0.8413) resolved via re-run (20260514: F1 = 0.7860 = 20260509 baseline bit-identically). Git audit confirms no training-code drift; root cause is stochastic non-determinism in CBSI step (HbT-specific).
> - ✅ **Decision logged: do NOT re-derive XAI on mt4.** The mt2 ranking already validated by 5/6 channel-ablation cells; competing mt4-derived ranking would complicate §III.C without changing the headline. Filed as Future Work only.
> - ✅ **Full breakdown** in `research/experiments/20260513/CHANNEL_ABLATION_RESULTS.md` (10 sections + K-consistency analysis §9.3); run commands in `CHANNEL_ABLATION_COMMANDS.md`; memory cross-refs updated.
> - ✅ **PAPER_OUTLINE.md** §3.3.3 + §4.1 + §5.2 bullet 13 + cross-cutting decisions table updated. **PAPER_SPEC_PLAN.md** Step 2 + Step 13 + Step 14 §III.C.7 + Step 16 V.A + V.B bullet 9 + D7 decision row all updated.
>
> **Progress log (2026-05-13 — P1.6 channel-to-channel differential analysis SHIPPED + narrative reframed):**
> - ✅ **P1.6 channel-to-channel class-differential probe** — `src/notebook/xai/05_channel_reduction.ipynb` (19 cells) + `scripts/build_xai_class_differential.py` + `research/xai/channel_reduction/`. **Headline:** S2_D1 (BA10-R, right frontopolar / vmPFC) is the top-1 differential channel in ST × HbO + ST × HbR — closes the §02 ↔ XAI disagreement (S2_D1 was rank-23 in *average* attention but rank-1 in *differential* `|M_diff|`). Structural-correlation confound rejected (ρ negative in all 3 cells). Cross-chromophore Kendall τ = +0.35, p = 0.019; cross-architecture n.s. Channel-ablation cuts K = 12 primary, K = 8 / 16 alternates handed off to the validation experiment.
> - ✅ **NARRATIVE REFRAMED 2026-05-13** — *Channel parsimony + network reshaping centred on S2_D1*, NOT hardware reduction. The 23 channels are built from 16 shared optodes (many-to-many mapping), so dropping channels does not proportionally drop optodes (K = 12 still needs 14 / 16 optodes). The paper claim is "every channel earns its place; class-discrimination is carried by a *reshaped* connectivity profile centred on S2_D1, not by amplifying / silencing single channels." The K = 8 / 12 / 16 sets are now positioned as **falsifiable ablation cuts** for the parsimony hypothesis, not minimal-montage recommendations.
> - ✅ **P1.7 reframed** as a falsifiable parsimony-hypothesis test. Both outcomes (Δ-F1 large negative ⇒ parsimony confirmed; Δ-F1 small ⇒ model robust to channel reduction) are publishable. Requires a new experiment module in the codebase (next-session work).
> - ✅ **PAPER_OUTLINE.md §3.3.3 + §5.2 bullet 13, PAPER_SPEC_PLAN.md Step 14 §III.C.7 + Step 16 Future Work bullet 9** all reframed.
> - ✅ **Label-convention bug fixed in the differential script**: `LABEL_MAP = {"healthy": 0, "anxiety": 1}` (per `src/core/dataset.py`), not the inverted mapping previously noted in `project_paper_outline_task_state.md`. All counts now reconcile with the headline metrics (Sens × 58 GAD trials = 58 TP; Spec × 66 HC trials = 40 TN ⇒ 98 correctly-classified for ST × HbO LOSO mt2).
>
> **Progress log (2026-05-11 session 6 — P1.2 concordance built):**
> - ✅ **P1.2 concordance triptych — SHIPPED.** `research/paper-materials/figures/results/concordance/concordance_triptych.{png,svg}` _(2026-05-15 reorg: moved from figures/concordance_triptych.{png,svg})_ (3×2 layout: ST attn HbO/HbR z-score topomaps + §02/§06 |d| topomaps + HbO/HbR temporal-attention lines) + `research/paper-materials/stats/concordance_rho_table.{csv,md}`. Build script `scripts/build_concordance_triptych.py` (reproducible from `src/.venv/bin/python` in ≤ 5 s). **Headline numbers:** Spearman ρ across 23 channels is null in all four XAI–stats pairings (HbO vs §02|d| = +0.00; HbO vs §06|βd| = −0.01; HbR vs §02|d| = +0.10; HbR vs §06|βd| = −0.05). **Top-10 overlap is the figure-of-merit:** ST-HbO ∩ §06|βd| = 6/10; C6 prior hits — ST-HbO 3/6, ST-HbR 4/6, §02 5/6, §06 5/6. **Bonus:** ρ(HbO XAI, HbR XAI) = +0.899 p<0.001 — chromophore-invariant attention surface.
...
>   - `figures/methods/architecture/fig1_overall_workflow.png` _(2026-05-15 reorg: moved from figures/)_ (graphical abstract; minor: EXPLAINABILITY arrow drawn twice, `ℝ` unicode slightly garbled)
>   - `figures/methods/architecture/fig2_sg_architecture.png` _(2026-05-15 reorg: moved from figures/)_ (clean, all hyperparameters accurate; pre-pool ordering wrong — see PAPER_ARCH_TABLES.md note)
>   - `figures/methods/architecture/fig3_st_architecture.png` _(2026-05-15 reorg: moved from figures/)_ (content accurate; box numbering shuffled — 5/6 swapped visually, but pipeline meaning unchanged)
>   - ~~`figures/fig4_montage.png`~~ _(2026-05-15 reorg: deleted — AI-generated, superseded by fig_montage_brain family in methods/montage/)_ (headcap + 5×7 grid; not pixel-accurate to channel coordinates — use as anatomical-feel inspiration only)
...
> - ✅ **P0.11 headline figures** — ~~`figures/fig5_headline_cm.png/.svg`~~ _(2026-05-15 reorg: deleted — old HbR K=23 headline F1=0.8406 superseded by HbO K=12 F1=0.8529; regenerate if needed for paper)_ (LOSO HbR mt2: TN=44, FP=22, FN=0, TP=58 → Acc 0.823, Sens 1.000, Spec 0.667, F1 0.8406 — matches Excel headline exactly) and ~~`figures/fig6_headline_training_grid.png/.svg`~~ _(2026-05-15 reorg: deleted — same supersedence as fig5)_ (5×3 grid: per-fold train+val curves for {Loss, Accuracy, F1} with best-epoch markers).
...
> - ✅ **P1.4 anatomical montage figure** — ~~`figures/fig7_montage_anatomical.{png,svg}`~~ + ~~`figures/fig7_montage_anatomical_data.csv`~~ _(2026-05-15 reorg: deleted — superseded by `figures/methods/montage/fig_optode_layout.{png,svg}` + the `figures/methods/montage/fig_montage_brain*.{png,svg}` family)_ (23 rows of channel midpoints in head-coordinate mm). Two-panel: (A) anatomically-accurate top-down scalp view with all 8 sources, 8 detectors, 23 channel midpoints, head outline + Nz/LPA/RPA fiducials + nose triangle, all rendered from `data/brainproducts-RNP-BA-128-custom.elc`; (B) the 5×7 grid mapping with every channel labeled at its (row, col) per `src/xai/channels.py:GRID_POS`. **This is the ground-truth montage figure** — the AI-generated ~~`fig4_montage.png`~~ _(also deleted)_ was illustrative only; use this one for the SPEC plan.
...
>    ```
>    research/paper-materials/figures/results/concordance/concordance_triptych.{png,svg}  # 2026-05-15 reorg
...
- **outputs.**
  - `research/paper-materials/figures/results/concordance/concordance_triptych.{png,svg}` _(2026-05-15 reorg)_
...
### P1.6. Channel-to-channel XAI: class-differential analysis ✅ DONE 2026-05-13 (narrative reframed 2026-05-13)
- **what.** Differential XAI probe `M_diff = M_GAD − M_HC` on top of the existing per-cell pair matrices. *Reframed narrative*: tests whether the GNN distinguishes HC vs GAD via a *reshaped* connectivity profile centred on a small subnetwork, and produces falsifiable channel-ablation cuts for a follow-up parsimony test. **Not a hardware-reduction analysis** — the 23-channel montage shares 16 optodes, so channel cuts do not yield proportional optode savings.
- **outputs.**
  - `src/notebook/xai/05_channel_reduction.ipynb` (19 cells): cumulative-mass curve on `|M_diff|` · cross-cell hub stability · structural-correlation confound audit · top-pair tables (signed) · discriminative-subnetwork figure · per-K channel-ablation cuts with optode-mapping disclosure (K = 8 / 12 / 16).
  - `scripts/build_xai_class_differential.py` — re-runs `explain_checkpoint` per cell, partitions `TrialAttribution` by `true_label`, aggregates per class. No `src/xai/` changes.
  - `research/xai/channel_reduction/` — 3 per-cell directories (`st_hbo/`, `st_hbr/`, `sg_hbo/`) with `channel_pair_matrix_{hc,gad,diff}.npy` + `per_trial_pair_matrices.npy` (for future bootstrap / re-aggregation) + `structural_C_diff.npy` + summary CSVs; plus shared `figures/`, `reduction_summary.csv`, `top_differential_pairs.csv`, `run.json`.
- **headline numbers.** Average GNN attention is near-uniform across 23 channels (K@80 % mass ≈ 21 in every cell — no channel is visibly redundant), BUT `|M_diff|` is concentrated (max / min weighted-degree ratio 2.4–2.9 ×). **S2_D1 (BA10-R, right frontopolar) is top-1 differential channel** in ST × HbO + ST × HbR (top-2 in SG × HbO) — closes the §02 ↔ XAI disagreement (S2_D1 was rank-23 in *average* attention). Robust pairs across all 3 cells: S2_D1–S3_D3 (GAD > HC), S2_D1–S3_D6 (HC > GAD), S2_D1–S6_D6 (GAD > HC). S2_D1's connectivity is *rewired* between groups, not amplified or silenced — the multivariate edge-level signal that justifies the graph model. Structural-correlation confound rejected: ρ (XAI `|M_diff|` w-deg, structural `|C_diff|` w-deg) is **negative** in all 3 cells (−0.33 / −0.28 / −0.39). Cross-chromophore Kendall τ = +0.35 (p = 0.019); cross-architecture τ ≈ +0.06 (n.s.). Optode disclosure (Threats-to-Validity only): K = 12 channel cut still needs 14 / 16 optodes; K = 8 → 11 / 16. *Channel ablation is not a hardware-reduction proposal*.

### P1.7. Channel-ablation validation of the parsimony hypothesis ✅ DONE 2026-05-14
- **what.** *Falsifiable hypothesis test*: does the 23-channel prefrontal montage carry minimally sufficient discriminative information? Retrained `WindowedSpatioTemporalGATNet` on the top-K channel subsets ranked by `|M_diff|` weighted degree (K = 8 / 12 / 16) from `05_channel_reduction.ipynb`, plus K=23 baseline, across HbO/HbR/HbT and mt2/mt4 = **24 runs**.
- **outcome.** **Parsimony hypothesis validated.** Locked paper headline: **ST × HbO × LOSO × mt=2 × K=12 = F1 0.8529** (sens 1.000, spec 0.697; +3.60 pp over HbO K=23 baseline; +1.23 pp over the prior global best HbR mt2 K=23 = 0.8406). K-consistency analysis: K=12 helps in 5/6 (chromo × mt) cells with mean Δ-F1 +2.52 pp — the most beneficial subset across configurations (vs +1.85 pp at K=16 and +1.29 pp at K=8). Secondary number: HbO mt4 K=8 = 0.8672 (highest raw F1; sens 0.957; reported as sensitivity-analysis row). HbT mt4 K=23 reproducibility anomaly resolved via re-run (0.8413 was a non-deterministic outlier; true baseline 0.7860).
- **code change.** `--channel_subset` flag added to `src/core_st/main.py` + `src/core_st/dataset.py` + `src/core_st/config.py`. 15 unit tests in `tests/core_st/test_channel_subset.py` (all pass; bit-identical to no-feature path when `channel_subset=None`).
- **artefacts.** `research/experiments/20260513/{mt2,mt4}/ST_GATv2_GNG_{hbo,hbr,hbt}_loso_mt{2,4}_noaug{_K08,_K12,_K16,}_2026051{3,4}/`. Full breakdown + analysis in `research/experiments/20260513/CHANNEL_ABLATION_RESULTS.md`. Run commands in `CHANNEL_ABLATION_COMMANDS.md`.
- **lands.** §III.C.2 ML results table (NEW headline + sensitivity-analysis row) · §III.C.7 final paragraph (parsimony validated, K-consistency claim) · §V.B Future Work bullet 9 (rewritten — was "channel-ablation validation"; now "K=4/K=6 collapse-point test + mt4-derived XAI re-derivation").
- **deferred to Future Work:** K=4/K=6 collapse-point test; mt4-XAI re-derivation (the existing channel-ablation results already validate the mt2 ranking; competing mt4-derived ranking would complicate §III.C without changing the headline); HbT mt4 variance characterisation (current n=3 sufficient for point estimate).
