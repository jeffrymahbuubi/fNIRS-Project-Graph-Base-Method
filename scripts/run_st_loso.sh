...
#
# Sweeps: max_trials (2, 4) × signal_type (hbo, hbr, hbt) = 6 runs total.
...
#
# Usage:
#   bash scripts/run_st_loso.sh                                # full sweep: mt={2,4} × signals={hbo,hbr,hbt}
#   bash scripts/run_st_loso.sh hbt                            # hbt only (still mt={2,4})
#   bash scripts/run_st_loso.sh hbo hbr                        # two signal types
#   bash scripts/run_st_loso.sh --max_trials=2                 # only mt2 (single value)
#   bash scripts/run_st_loso.sh --max_trials=2,4 hbt           # explicit mt sweep on hbt
#   bash scripts/run_st_loso.sh --checkpoint_metric=loss       # checkpoint on lowest val loss
#   bash scripts/run_st_loso.sh --scheduler=cosine_warmup      # override default cosine_annealing
#   bash scripts/run_st_loso.sh --eta_min=0.0                  # override default eta_min=1e-5
...
for MAX_TRIALS in "${MAX_TRIALS_LIST[@]}"; do
    for DATA_TYPE in "${DATA_TYPES[@]}"; do
        ...
        echo ""
        echo "=========================================="
        echo "LOSO | max_trials: $MAX_TRIALS | Signal: $DATA_TYPE | epochs: $EPOCHS | lr: $LR | scheduler: $SCHEDULER | eta_min: $ETA_MIN"
        echo "=========================================="
